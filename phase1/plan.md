...
- [ ] `P1-B1` Add explicit `typecheck`, `test`, `lint` scripts in `packages/core/package.json`.
- [ ] `P1-B2` Add explicit `typecheck`, `test`, `lint` scripts in `packages/cli/package.json`.
- [ ] `P1-B3` Install and wire missing test tooling (Vitest/TS runner) so tests actually execute.
- [ ] `P1-B4` Make root scripts fail if package scripts are missing (remove `--if-present` for required checks).
